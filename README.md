...
Traditional routeing loads pages from the server. So a request is sent to the server and that information is returned. This means that the browser gets refreshed with incomming information. Everything on the dom gets dropped and reinstalled with new information, even if information did not change. It all gets wiped and reinstalled. 

With client side routing, routing is handled on the client machine, not on some machine somewhere else. This allows for keeping information that doesn't need to be refreshed. With client side routing, only the infomation that needs to be changed can be changed and is local to the system. This removes the flash of reload, delay of load, etc.
